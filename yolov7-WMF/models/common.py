...
class Mycontact4(nn.Module):
    # WMF 冻结版实现：4 路可学习加权融合
    def __init__(self, in_chs, out_ch, dimension=1):
        super(Mycontact4, self).__init__()
        assert len(in_chs) == 4
        self.d = dimension
        self.w = nn.Parameter(torch.ones(4), requires_grad=True)
        self.cv = Conv(sum(in_chs), out_ch, 1, 1)

    def forward(self, xs):
        w = self.w / (self.w.sum() + 1e-4)
        return self.cv(torch.cat([w[0] * xs[0], w[1] * xs[1], w[2] * xs[2], w[3] * xs[3]], self.d))


class Mycontact6(nn.Module):
    # WMF 冻结版实现：6 路可学习加权融合
    def __init__(self, in_chs, out_ch, dimension=1):
        super(Mycontact6, self).__init__()
        assert len(in_chs) == 6
        self.d = dimension
        self.w = nn.Parameter(torch.ones(6), requires_grad=True)
        self.cv = Conv(sum(in_chs), out_ch, 1, 1)

    def forward(self, xs):
        w = self.w / (self.w.sum() + 1e-4)
        return self.cv(torch.cat([w[0] * xs[0], w[1] * xs[1], w[2] * xs[2], w[3] * xs[3], w[4] * xs[4], w[5] * xs[5]], self.d))


class Multicat(nn.Module):
    # WMF 冻结版实现：仅对 xs[0] 做池化求和，再与其余分支融合
    def __init__(self, in_chs, out_ch, dimension=1):
    ...
    def forward(self, xs):
        x0 = self.avgpool(xs[0]) + self.maxpool(xs[0])
        return self.cv(torch.cat([x0] + list(xs[1:]), self.d))
...
class WMFSemSegHead(nn.Module):
    def __init__(self, in_chs, inner_ch=128, dec_chs=(64, 32, 16), out_ch=1):
        super(WMFSemSegHead, self).__init__()
        c3, c4, c5 = in_chs
        self.p3_proj = Conv(c3, inner_ch, 1, 1)
        self.p4_proj = Conv(c4, inner_ch, 1, 1)
        self.p5_proj = Conv(c5, inner_ch, 1, 1)
        self.fuse4 = nn.Sequential(
            Conv(inner_ch * 2, inner_ch, 3, 1),
            Conv(inner_ch, inner_ch, 3, 1)
        )
        ...
        self.dec1 = nn.Sequential(
            Conv(inner_ch, dec_chs[0], 3, 1),
            Conv(dec_chs[0], dec_chs[0], 3, 1)
        )
        self.dec2 = nn.Sequential(
            Conv(dec_chs[0], dec_chs[1], 3, 1),
            Conv(dec_chs[1], dec_chs[1], 3, 1)
        )
        self.dec3 = nn.Sequential(
            Conv(dec_chs[1], dec_chs[2], 3, 1),
            Conv(dec_chs[2], dec_chs[2], 3, 1)
        )
        self.out = nn.Conv2d(dec_chs[2], out_ch, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, xs):
        ...
        f4 = self.fuse4(torch.cat([p4, p5_up], dim=1))
        f4_up = F.interpolate(f4, size=p3.shape[-2:], mode='nearest')
        x = self.fuse3(torch.cat([p3, f4_up], dim=1))
